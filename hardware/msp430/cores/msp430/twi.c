...
/*
//REMOVED 7.dec.2014 and moved to usci_isr_handler.h
...
void twi_init(void)
{
    ...
    UCBxCTLW1 &= ~UCASTP_3;
    //UCBxCTLW1 |= autoSTOPGeneration;

    //Byte Count Threshold
    //UCBxTBCNT = byteCounterThreshold;
    /*
...
uint8_t twi_readFrom(uint8_t address, uint8_t* data, uint8_t length, uint8_t sendStop)
{
    ...
    UC0IE |= (UCBxRXIE | UCBxTXIE);          // Enable I2C interrupts
#else
    ...
    UCBxCTLW0 |= (UCMST | UCMODE_3 | UCSYNC | UCSSEL__SMCLK);   // I2C Master, synchronous mode
    UCBxCTLW0 &= ~(UCTR);                     // Configure in receive mode
    UCBxI2CSA = address;                      // Set Slave Address
    UCBxCTLW0 &= ~UCSWRST;                    // Clear SW reset, resume operation
    UCBxIE |= (UCRXIE0|UCALIE|UCNACKIFG|UCSTTIFG|UCSTPIFG); // Enable I2C interrupts
#endif
...
#ifdef __MSP430_HAS_USCI__
    UCBxI2CIE |= (UCALIE|UCNACKIE|UCSTPIE);  // Enable I2C interrupts
    UC0IE |= UCBxTXIE;                     // Enable I2C interrupts
#else
...
#ifdef __MSP430_HAS_EUSCI_B0__
    UCBxCTLW0 = UCSWRST;                      // Enable SW reset
    UCBxCTLW0 |= (UCMST | UCMODE_3 | UCSSEL__SMCLK | UCSYNC);   // I2C Master, synchronous mode
    UCBxCTLW0 |= UCTR;                        // Configure in transmit mode
    UCBxI2CSA = address;                      // Set Slave Address
    UCBxCTLW0 &= ~UCSWRST;                    // Clear SW reset, resume operation
    UCBxIE |= (UCTXIE0|UCALIE|UCNACKIE|UCSTPIE); // Enable I2C interrupts
#endif
	if(length == 0) {
		return 0;
	}
