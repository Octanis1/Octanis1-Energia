...
void USCIB0_ISR(void)
{
	...
	if ((UCBxCTL0 & UCMODE_3) == UCMODE_3 && (UCBxIFG & (UCALIFG | UCNACKIFG | UCSTTIFG | UCSTPIFG)) != 0)
		i2c_state_isr(); 
	/* USCI_B0 I2C TX RX interrupt. */
	if ((UCBxCTL0 & UCMODE_3) == UCMODE_3 && (UCBxIFG & (UCTXIFG | UCRXIFG)) != 0)
		i2c_txrx_isr();
 ...
 * I2C: 
 *	USCIAB0TX_VECTOR services both UCBxTXIFG and UCBxRXIFG
 *	set in UC0IFG.
 *	USCIAB0RX_VECTOR services the state change interrupts
 *	UCSTTIFG, UCSTPIFG, UCIFG, UCALIFG set in UCBxSTAT.*/
...
void USCIAB0TX_ISR(void)
{
	...
	/* USCI_B0 I2C TX RX interrupt. */
	if ((UCBxCTL0 & UCMODE_3) == UCMODE_3 && (UC0IFG & (UCBxTXIFG | UCBxRXIFG)) != 0)
		i2c_txrx_isr();
...
void USCIAB0RX_ISR(void)
{
	...
	/* USCI_B0 I2C state change interrupt. */
	if ((UCBxSTAT & (UCALIFG | UCNACKIFG | UCSTTIFG | UCSTPIFG)) != 0)
		i2c_state_isr(); 
